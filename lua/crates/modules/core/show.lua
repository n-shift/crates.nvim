...
local get_dependencies = function(unstable)
	local cargo_toml = toml.parse(table.concat(vim.api.nvim_buf_get_lines(0, 0, -1, false), "\n"), { strict = false })
	local dependencies = {}
	...
	for name, metadata in pairs(deps) do
		if metadata.version then
			...
			local res, _ = pcall(semver, metadata)
			if res then
...
local get_outdated_deps = function(list, unstable)
	...
	for name, version in pairs(list) do
		...
		if semver(version) < semver(newest) then
			deps[name] = newest
		end
	end
	return deps
...
return function()
	if not is_valid_cargo_toml() then
		return
	end
